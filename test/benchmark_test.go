...
func TestBenchmark(t *testing.T) {
	...
	t.Run("short messages", func(t *testing.T) {
		...
		result := testing.Benchmark(func(b *testing.B) {
			for i := 0; i < b.N; i++ {
				assert.NoError(b, clients[0].SendMsg(srv.ListClientIDs(0), payload))
...
func waitForClientsToConnect(t *testing.T, srv *server.Server) {
	for i := 0; i < 50; i++ {
		if clientCount != len(srv.ListClientIDs(0)) {
	...
	require.Equal(t, clientCount, len(srv.ListClientIDs(0)))
}
